...
categorias.forEach(category => {
    category.addEventListener('click', function () {
        ...
        if (!category.classList.contains('active')) {
            ...
            if (category.getAttribute('data-values').trim() !== "") {
...
function subcategoriaClick(valueElement, category) {
    if (!valueElement.classList.contains('active')) {
        ...
        filterPagesSubCategory(category.textContent.trim().toLowerCase(), valueElement.textContent.toLowerCase());
    } else {
        deselectSubCategoriesButtons();
        filterPagesByCategory(category.textContent.trim().toLowerCase());
    }
...
function filterPagesByCategory(category) {
    ...
    pages.forEach(async page => {
        ...
        const dataCategoriesNormal = page.getAttribute('data-categories');

        if (dataCategoriesNormal && dataCategoriesNormal != "null") {

            const dataCategories = dataCategoriesNormal.toLocaleLowerCase();

            const pageCategories = JSON.parse(dataCategories);

            if (pageCategories.hasOwnProperty(category)) {
...
function filterPagesSubCategory(category, value) {
    ...
    pages.forEach(async page => {
        ...
        if (dataCategoriesNormal && dataCategoriesNormal != "null") {
            ...
            if (pageCategories.hasOwnProperty(category) && pageCategories[category].includes(value)) {
                page.style.display = 'block';
            } else {
                page.style.display = 'none';
